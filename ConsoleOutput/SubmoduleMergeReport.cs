    ...
    private static void PrintTableBody(PrintableSuperprojectDto printableSuperprojectDto, Dictionary<string, TableColumn> columns)
    {
        ...
        foreach (string branch in printableSuperprojectDto.RevelantBranches)
        {
            ...
            foreach (string submoduleName in printableSuperprojectDto.Submodules)
            {
                ...
                // Submodule IndexCommmit SubmoduleRepoHeadCommit
                Console.WriteLine(
                        columns[Column.Submodule].GetPrintableValue(submoduleName, columns[Column.SuperProject].Width + columns[Column.Branch].Width + Delimeter.Length * 2) + Delimeter +
                        superprojectIndexes[submoduleName] + Delimeter +
                        submoduleHeadCommits[submoduleName] 
                    );
            }
        }
    }
